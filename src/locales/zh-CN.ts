export default {
  ...
  login: {
    ...
    oidcUnbound: '该 OIDC 账号未绑定用户，请先使用用户名密码登录后绑定',
    oidcError: 'OIDC 认证失败',
    userInactive: '用户已被禁用',
    loggingIn: '正在登录',
    pleaseWait: '请稍候...',
  ...
  settingTabs: {
    ...
    oidc: {
      title: 'OIDC 登录',
      description: 'OIDC 签发者、客户端配置、回调地址、连接测试',
    },
    about: {
      title: '关于',
      description: '软件版本',
  ...
  user: {
    ...
    noEmail: '未设置邮箱',
    oidcBound: '已绑定OIDC',
    oidcBoundWith: '已绑定 {sub}',
    movieSubscriptions: '电影订阅',
    tvSubscriptions: '剧集订阅',
    cannotDeleteCurrentUser: '不能删除当前登录用户！',
    bindOidc: '绑定OIDC账号',
    bindOidcSuccess: 'OIDC账号绑定成功',
    bindOidcFailed: 'OIDC账号绑定失败',
    unbindOidc: '解绑OIDC账号',
    confirmUnbindOidc: '确定要解绑OIDC账号吗？解绑后将无法使用OIDC登录。',
    unbindOidcSuccess: 'OIDC账号解绑成功',
    unbindOidcFailed: 'OIDC账号解绑失败',
  ...
  setting: {
    ...
    oidc: {
      title: 'OIDC 登录',
      description: '配置 OpenID Connect 登录，允许通过外部身份提供商进行认证登录',
      issuer: '签发者 URL (Issuer)',
      issuerPlaceholder: 'https://your-idp.example.com/realms/master',
      issuerHint: 'OIDC 提供商的签发者 URL',
      clientId: '客户端 ID',
      clientIdPlaceholder: 'your-client-id',
      clientSecret: '客户端密钥',
      clientSecretPlaceholder: 'your-client-secret',
      scopes: '请求的 Scopes',
      scopesHint: '空格分隔的 OAuth2 scope 列表，通常为 openid profile email',
      redirectUri: '回调地址 (Redirect URI)',
      redirectUriHint: 'OIDC 提供商中配置的回调地址，留空则自动根据当前访问地址生成',
      redirectUriCopy: '复制回调地址',
      enabledStatus: 'OIDC 登录已启用，用户可使用 OIDC 账号登录',
      disabledStatus: 'OIDC 登录未启用',
      testConnection: '测试连接',
      testNotSaved: '请先保存设置后再测试',
      testDiscoveryFailed: '发现文档获取失败 (HTTP {status})',
      testInvalidDiscovery: '发现文档缺少必要字段 (authorization_endpoint 或 token_endpoint)',
      testSuccess: '连接测试成功，发现文档获取正常',
      testError: '测试失败: {error}',
      issuerRequired: '请先填写签发者 URL',
      saveSuccess: 'OIDC 设置保存成功',
      saveFailed: 'OIDC 设置保存失败',
      usageGuide: '使用指南',
      guideStep1: '1. 在您的 OIDC 提供商（如 Keycloak、Authentik、Okta 等）中创建一个 OIDC 客户端',
      guideStep2: '2. 将客户端的回调地址设置为以下地址（点击可复制）：',
      guideStep3: '3. 填写签发者 URL、客户端 ID 和客户端密钥',
      guideStep4: '4. 保存设置后，登录页面将显示 OIDC 登录按钮',
      guideStep5: '5. 已登录用户可在 个人信息 → 双重验证 → OIDC 中绑定/解绑账号',
      serverConfig: '服务配置',
      accountBinding: '账号绑定',
    },
    words: {
  ...
  profile: {
    ...
    doubanUser: '豆瓣用户',
    oidcUser: 'OIDC 账号',
    oidcUserPlaceholder: '未绑定',
    oidcNotEnabled: '管理员未启用 OIDC 登录，请联系管理员开启',
